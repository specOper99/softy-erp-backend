...
| Payroll run + payroll history | `ADMIN` on `/[tenantId]/payroll` | `POST /hr/payroll/run` with optional `month` and `year`; `GET /hr/payroll/history`; `GET /hr/payroll/history/cursor` | Payroll run response and payroll history rows normalized to UI fields | Manual payroll runs must accept explicit period payload; history rows must normalize `totalPayout` / `processedAt` for the UI | `frontend/tests/finance-contracts.test.ts`; `backend/src/modules/hr/api/hr.controller.spec.ts`; `backend/src/modules/hr/application/payroll.service.spec.ts` |
