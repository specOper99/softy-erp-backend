...
import { BookingStatus, ReferenceType, TaskStatus, TransactionType } from '../../common/enums';
import { AuditService } from '../audit/audit.service';
import { ServicePackage } from '../catalog/entities/service-package.entity';
import { FinanceService } from '../finance/services/finance.service';
import { MailService } from '../mail/mail.service';
import { Task } from '../tasks/entities/task.entity';
import { ConfirmBookingResponseDto, CreateBookingDto, UpdateBookingDto } from './dto';
...
export class BookingsService {
    ...
    async create(dto: CreateBookingDto): Promise<Booking> {
        ...
        if (!pkg) {
            throw new NotFoundException(`ServicePackage with ID ${dto.packageId} not found`);
        }
    ...
    async findOne(id: string): Promise<Booking> {
        const booking = await this.bookingRepository.findOne({
            where: { id },
            relations: ['servicePackage', 'servicePackage.packageItems', 'servicePackage.packageItems.taskType', 'tasks', 'tasks.assignedUser', 'tasks.taskType'],
        });
        if (!booking) {
