...
describe('BookingWorkflowService', () => {
    ...
    it('should reschedule booking when there is no conflict', async () => {
      ...
      expect(queryRunner.manager.save).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          payload: expect.objectContaining({
            bookingId: 'booking-1',
            staffEmails: expect.arrayContaining([
              'legacy-user-1@example.com',
              'user-a@example.com',
              'user-b@example.com',
            ]),
          }),
  ...
  describe('completeBooking', () => {
    it('publishes BookingCompletedEvent once after completion commit', async () => {
      ...
      expect(queryRunner.manager.save).toHaveBeenCalledWith(
        OutboxEvent,
        expect.objectContaining({ type: 'BookingCompletedEvent', aggregateId: 'booking-1' }),
      );
  ...
  describe('duplicateBooking', () => {
    it('writes BookingCreatedEvent outbox row and publishes CQRS event after duplicate commit', async () => {
      ...
      queryRunner.manager.create = mockCreate;
      (queryRunner.manager.findOne as jest.Mock).mockResolvedValue(mockBooking);
      (queryRunner.manager.save as jest.Mock).mockImplementation((targetOrEntity: unknown, maybeEntity: unknown) => {
        if (targetOrEntity === Booking) {
          return Promise.resolve(savedDuplicate);
        }
        return Promise.resolve(maybeEntity || targetOrEntity);
      });
      ...
      expect(dataSource.transaction).toHaveBeenCalled();
      expect(queryRunner.manager.findOne).toHaveBeenCalledWith(Booking, {
