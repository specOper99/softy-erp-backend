...
import { parseCanonicalBookingDateInput } from '../domain/utils/booking-date-policy.util';
import { applyBookingFilters } from '../domain/utils/booking-query-builder.util';
...
export class BookingsService {
  ...
  async create(dto: CreateBookingDto): Promise<Booking> {
    ...
    const savedBooking = await this.dataSource.transaction(async (manager) => {
      let saved = await manager.save(Booking, booking);

      // Attach processing types via join table if provided
      if (dto.processingTypeIds && dto.processingTypeIds.length > 0) {
        saved.processingTypes = selectedProcessingTypes;
        saved = await manager.save(Booking, saved);
      } else {
        saved.processingTypes = [];
      }
  ...
  async update(id: string, inputDto: UpdateBookingDto, user: User): Promise<Booking> {
    ...
    const savedBooking = await this.dataSource.transaction(async (manager) => {
      ...
      const oldSubTotal = originalPricing.subTotal;
      const oldTaxAmount = originalPricing.taxAmount;
      const oldTotalPrice = originalPricing.totalPrice;
      const newSubTotal = Number(saved.subTotal || 0);
      const newTaxAmount = Number(saved.taxAmount || 0);
      const newTotalPrice = Number(saved.totalPrice || 0);
