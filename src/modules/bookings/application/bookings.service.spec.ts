...
import { PaymentStatus } from '../../finance/domain/enums/payment-status.enum';
import type { User } from '../../users/domain/entities/user.entity';
import { Role } from '../../users/domain/enums/role.enum';
import type { CreateBookingDto } from '../api/dto';
...
describe('BookingsService', () => {
  ...
  describe('create', () => {
    ...
    it('writes BookingCreatedEvent through the create transaction manager', async () => {
      const dto: CreateBookingDto = {
        clientId: 'client-1',
        packageId: 'pkg-1',
        eventDate: new Date(Date.now() + 86400000).toISOString(),
      };
      const managerSave = jest.fn().mockImplementation((targetOrEntity: unknown) => {
        if (targetOrEntity === OutboxEvent) return Promise.resolve({});
      ...
      expect(managerSave).toHaveBeenCalledWith(Booking, mockBooking);
      expect(managerSave).toHaveBeenCalledWith(
        OutboxEvent,
        expect.objectContaining({
          type: 'BookingCreatedEvent',
          aggregateId: mockBooking.id,
    ...
    it('should attach processing types when processingTypeIds provided', async () => {
      ...
      const createdBooking = { ...mockBooking, processingTypes: [] };
      const savedBooking = { ...mockBooking, processingTypes: [mockProcessingType] };
      const managerSave = jest.fn().mockImplementation((targetOrEntity: unknown) => {
        if (targetOrEntity === OutboxEvent) return Promise.resolve({});
        return Promise.resolve(savedBooking);
      });
      bookingRepository.create.mockReturnValue(createdBooking);
      dataSource.transaction.mockImplementation((cb) =>
        cb({
          save: managerSave,
        }),
      ...
      // save is called twice for booking aggregate, then once for outbox row
      expect(managerSave).toHaveBeenCalledWith(Booking, createdBooking);
      expect(managerSave).toHaveBeenCalledWith(
        Booking,
        expect.objectContaining({ processingTypes: [mockProcessingType] }),
      );
      expect(managerSave).toHaveBeenCalledWith(
        OutboxEvent,
        expect.objectContaining({ type: 'BookingCreatedEvent', aggregateId: savedBooking.id }),
      );
      expect(outboxRepository.save).not.toHaveBeenCalled();
    });
  ...
  describe('update', () => {
    ...
    it('publishes only BookingUpdatedEvent when pricing does not change', async () => {
      ...
      expect(managerSave).not.toHaveBeenCalledWith(
        OutboxEvent,
        expect.objectContaining({ type: 'BookingPriceChangedEvent' }),
      );
      expect(outboxRepository.save).not.toHaveBeenCalled();
    });
  });
