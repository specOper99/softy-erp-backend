...
import { BookingIntakeDto, BookingIntakeResponseDto } from '../api/dto/booking-intake.dto';
...
import { PaymentRecordedEvent } from '../domain/events/payment-recorded.event';
...
import { BookingPriceCalculator } from '../domain/utils/booking-price.calculator';
  ...
  async intake(dto: BookingIntakeDto): Promise<BookingIntakeResponseDto> {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await manager.save(OutboxEvent, {
        ...
        tenantId,
        occurredAt: savedBookingCreatedAt,
        payload: {
          ...
          assignedUserId: null,
          eventDate,
          createdAt: savedBookingCreatedAt,
        },
      });
      ...
      if (dto.deposit && dto.deposit.amount > 0) {
        ...
        await manager.save(OutboxEvent, {
          ...
          payload: {
            ...
            amount: dto.deposit.amount,
            paymentMethod: dto.deposit.paymentMethod,
            reference: dto.deposit.reference ?? '',
            totalPrice: pricing.totalPrice,
