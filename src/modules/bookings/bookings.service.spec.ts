...
describe('BookingsService - Comprehensive Tests', () => {
    ...
    const mockPackage = {
        ...
        isActive: true,
        packageItems: [
            { taskTypeId: 'task-type-1', quantity: 2, taskType: { defaultCommissionAmount: 100 } },
            { taskTypeId: 'task-type-2', quantity: 1, taskType: { defaultCommissionAmount: 150 } },
        ],
    };
    ...
    beforeEach(async () => {
        ...
        mockBookingRepository.findOne.mockImplementation(({ where }) => {
            if (where.id === 'booking-uuid-123') return Promise.resolve({ ...mockBooking });
            return Promise.resolve(null);
        });
    });
    ...
    describe('update', () => {
        it('should update draft booking client name', async () => {
            const result = await service.update('booking-uuid-123', { clientName: 'Jane Doe' });
            expect(mockBookingRepository.save).toHaveBeenCalled();
        });

        it('should update draft booking notes', async () => {
            const result = await service.update('booking-uuid-123', { notes: 'Updated notes' });
            expect(mockBookingRepository.save).toHaveBeenCalled();
        });

        it('should update draft booking event date', async () => {
            const result = await service.update('booking-uuid-123', { eventDate: '2025-01-15T10:00:00Z' });
            expect(mockBookingRepository.save).toHaveBeenCalled();
        });
    ...
    describe('remove', () => {
        ...
        it('should reject deleting confirmed booking', async () => {
            ...
            await expect(service.remove('booking-uuid-123')).rejects.toThrow(BadRequestException);
        });
    ...
    describe('confirmBooking', () => {
        ...
        it('should reject confirming already confirmed booking', async () => {
            ...
            await expect(service.confirmBooking('booking-uuid-123')).rejects.toThrow(BadRequestException);
        });
        ...
        it('should rollback on transaction failure', async () => {
            ...
            await expect(service.confirmBooking('booking-uuid-123')).rejects.toThrow('Transaction failed');
    ...
    describe('cancelBooking', () => {
        ...
        it('should reject cancelling already cancelled booking', async () => {
            ...
            await expect(service.cancelBooking('booking-uuid-123')).rejects.toThrow(BadRequestException);
        });
    ...
    describe('completeBooking', () => {
        ...
        it('should reject completing draft booking', async () => {
            await expect(service.completeBooking('booking-uuid-123')).rejects.toThrow(BadRequestException);
        });
