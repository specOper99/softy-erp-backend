...
describe('HrService - Comprehensive Tests', () => {
    ...
    describe('createProfile', () => {
        it('should create profile and wallet for user', async () => {
            ...
            expect(mockFinanceService.getOrCreateWallet).toHaveBeenCalledWith('user-uuid-123');
    ...
    describe('findAllProfiles', () => {
        it('should return all profiles with user relations', async () => {
            ...
            expect(mockProfileRepository.find).toHaveBeenCalledWith({ relations: ['user'] });
        });
    ...
    describe('updateProfile', () => {
        it('should update profile first name', async () => {
            const result = await service.updateProfile('profile-uuid-123', { firstName: 'Jane' });
            expect(mockProfileRepository.save).toHaveBeenCalled();
        });

        it('should update profile salary', async () => {
            const result = await service.updateProfile('profile-uuid-123', { baseSalary: 3000 });
            expect(mockProfileRepository.save).toHaveBeenCalled();
        });

        it('should update profile job title', async () => {
            const result = await service.updateProfile('profile-uuid-123', { jobTitle: 'Senior Photographer' });
            expect(mockProfileRepository.save).toHaveBeenCalled();
        });
    ...
    describe('runPayroll', () => {
        ...
        it('should skip employees with zero payout', async () => {
            ...
            mockQueryRunner.manager.findOne.mockResolvedValueOnce({ ...mockWallet, payableBalance: 0 });
        ...
        it('should process multiple employees', async () => {
            const employee2 = { ...mockProfile, id: 'profile-2', userId: 'user-2', baseSalary: 3000 };
            mockQueryRunner.manager.find.mockResolvedValueOnce([mockProfile, employee2]);
        ...
        it('should handle high salary and commission values', async () => {
            ...
            mockQueryRunner.manager.findOne.mockResolvedValueOnce({
                ...mockWallet, payableBalance: 99999.99,
            });
