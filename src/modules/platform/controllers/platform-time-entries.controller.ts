...
export class PlatformTimeEntriesController {
  ...
  list(@TargetTenant() tenantId: string, @Query() query: PlatformTimeEntryQueryDto) {
    return TenantContextService.run(tenantId, async () => this.service.list(tenantId, query));
  }
  ...
  findOne(@Param('id', ParseUUIDPipe) id: string, @TargetTenant() tenantId: string) {
    return TenantContextService.run(tenantId, async () => this.service.findOne(tenantId, id));
  }
  ...
  update(
    ...
    return TenantContextService.run(tenantId, async () =>
      this.service.update(tenantId, id, dto, req.user.userId, req.ip, req.headers['user-agent']),
    );
  }
}
