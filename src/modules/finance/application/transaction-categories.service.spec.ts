...
import type { TransactionCategory } from '../domain/entities/transaction-category.entity';
...
describe('TransactionCategoriesService', () => {
  ...
  describe('findById', () => {
    ...
    it('does not expose category id owned by another tenant', async () => {
      // TenantAwareRepository scopes findOne by tenantId; cross-tenant rows are invisible.
      repository.findOne.mockResolvedValue(null);

      await expect(service.findById('category-owned-by-other-tenant')).rejects.toThrow(NotFoundException);
      expect(repository.findOne).toHaveBeenCalledWith({
        where: { id: 'category-owned-by-other-tenant' },
        relations: ['parent', 'children'],
      });
    });
  });
