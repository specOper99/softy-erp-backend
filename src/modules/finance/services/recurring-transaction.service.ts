...
export class RecurringTransactionService {
  ...
  async findAllCursor(
    ...
    if (query.cursor) {
      ...
      qb.andWhere(
        '(rt.createdAt < :date OR (rt.createdAt = :date AND rt.id < :id))',
        { date, id },
      );
    }
  ...
  async processDueTransactions() {
    ...
    if (!acquired) {
      this.logger.log(
        'Skipping recurring transactions - another instance holds the lock',
      );
      return;
    }
  ...
  private async processTransaction(rt: RecurringTransaction) {
    try {
      ...
      if (rt.failureCount >= 3) {
        ...
        this.logger.error(
          `Recurring transaction ${rt.id} permanently failed after 3 attempts: ${rt.lastError}`,
        );
      }

      await this.recurringRepo.save(rt);
      this.logger.error(
        `Failed to process recurring transaction ${rt.id} (attempt ${rt.failureCount})`,
        error,
      );
    }
  }
