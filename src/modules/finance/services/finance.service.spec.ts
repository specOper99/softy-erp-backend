...
describe('FinanceService - Comprehensive Tests', () => {
    ...
    const mockTransactionRepository = {
        ...
        save: jest.fn().mockImplementation((txn) => Promise.resolve({ id: 'txn-uuid-123', ...txn })),
        findOne: jest.fn(),
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(EmployeeWallet), useValue: mockWalletRepository },
        ...
        mockWalletRepository.findOne.mockImplementation(({ where }) => {
            if (where.userId === 'user-uuid-123') return Promise.resolve({ ...mockWallet });
            return Promise.resolve(null);
        });
    });
    ...
    describe('findAllTransactions', () => {
        ...
        it('should filter by transaction type', async () => {
            const result = await service.findAllTransactions({ type: TransactionType.INCOME });
    ...
    describe('getOrCreateWallet', () => {
        it('should return existing wallet', async () => {
            ...
            expect(result).toEqual(expect.objectContaining({ userId: 'user-uuid-123' }));
        });

        it('should create new wallet if not exists', async () => {
            mockWalletRepository.findOne.mockResolvedValueOnce(null);

            const result = await service.getOrCreateWallet('new-user-uuid');
    ...
    describe('addPendingCommission', () => {
        it('should add commission to pending balance', async () => {
            const walletCopy = { ...mockWallet, pendingBalance: 100, payableBalance: 200 };
            const mockManager = {
            ...
            const result = await service.addPendingCommission(mockManager as any, 'user-uuid-123', 50.0);
        ...
        it('should create wallet if not exists', async () => {
            ...
            const result = await service.addPendingCommission(mockManager as any, 'new-user', 50.0);
        ...
        it('should handle zero commission', async () => {
            ...
            const result = await service.addPendingCommission(mockManager as any, 'user-uuid-123', 0);
    ...
    describe('moveToPayable', () => {
        it('should move amount from pending to payable', async () => {
            ...
            const result = await service.moveToPayable(mockManager as any, 'user-uuid-123', 50.0);
    ...
    describe('resetPayableBalance', () => {
        it('should reset payable balance to zero', async () => {
            ...
            const result = await service.resetPayableBalance(mockManager as any, 'user-uuid-123');
    ...
    describe('createTransactionWithManager', () => {
        it('should create transaction using entity manager', async () => {
            const mockManager = {
                create: jest.fn().mockImplementation((Entity, data) => data),
                save: jest.fn().mockImplementation((txn) => Promise.resolve({ id: 'txn-new', ...txn })),
            };
            ...
            const result = await service.createTransactionWithManager(mockManager as any, dto);
            expect(mockManager.create).toHaveBeenCalled();
