...
describe('CatalogService', () => {
    ...
    const mockTaskType = {
        ...
        defaultCommissionAmount: 100.00,
        isActive: true,
        createdAt: new Date(),
        updatedAt: new Date(),
    };
    ...
    const mockPackageRepository = {
        ...
        save: jest.fn().mockImplementation((pkg) => Promise.resolve({ id: 'pkg-uuid-123', ...pkg })),
    ...
    const mockTaskTypeRepository = {
        create: jest.fn().mockImplementation((dto) => dto),
        save: jest.fn().mockImplementation((tt) => Promise.resolve({ id: 'task-type-uuid-123', ...tt })),
        find: jest.fn().mockResolvedValue([mockTaskType]),
        findOne: jest.fn().mockImplementation(({ where }) => {
            if (where.id === 'task-type-uuid-123') return Promise.resolve(mockTaskType);
            return Promise.resolve(null);
        }),
    ...
    const mockPackageItemRepository = {
        create: jest.fn().mockImplementation((dto) => dto),
        save: jest.fn().mockImplementation((item) => Promise.resolve({ id: 'item-uuid-123', ...item })),
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                CatalogService,
                { provide: getRepositoryToken(ServicePackage), useValue: mockPackageRepository },
                { provide: getRepositoryToken(TaskType), useValue: mockTaskTypeRepository },
                { provide: getRepositoryToken(PackageItem), useValue: mockPackageItemRepository },
    ...
    describe('ServicePackage CRUD', () => {
        ...
        describe('findPackageById', () => {
            ...
            it('should throw NotFoundException for invalid id', async () => {
                await expect(service.findPackageById('invalid-id')).rejects.toThrow(NotFoundException);
            });

            it('should throw NotFoundException for empty string id', async () => {
                await expect(service.findPackageById('')).rejects.toThrow(NotFoundException);
            });
        });

        describe('updatePackage', () => {
            it('should update package name', async () => {
                const result = await service.updatePackage('pkg-uuid-123', { name: 'Updated Name' });
                expect(mockPackageRepository.save).toHaveBeenCalled();
            });

            it('should update package price', async () => {
                const result = await service.updatePackage('pkg-uuid-123', { price: 2000 });
                expect(mockPackageRepository.save).toHaveBeenCalled();
            });
        ...
        describe('deletePackage', () => {
            ...
            it('should throw NotFoundException when deleting non-existent package', async () => {
                await expect(service.deletePackage('invalid-id')).rejects.toThrow(NotFoundException);
            });
        });
    });
    ...
    describe('TaskType CRUD', () => {
        ...
        describe('findTaskTypeById', () => {
            ...
            it('should throw NotFoundException for invalid task type id', async () => {
                await expect(service.findTaskTypeById('invalid-id')).rejects.toThrow(NotFoundException);
            });
        });

        describe('updateTaskType', () => {
            it('should update task type commission', async () => {
                const result = await service.updateTaskType('task-type-uuid-123', { defaultCommissionAmount: 150 });
                expect(mockTaskTypeRepository.save).toHaveBeenCalled();
            });

            it('should update task type name', async () => {
                const result = await service.updateTaskType('task-type-uuid-123', { name: 'New Name' });
                expect(mockTaskTypeRepository.save).toHaveBeenCalled();
            });
        });
    });
    ...
    describe('PackageItem Management', () => {
        describe('addPackageItems', () => {
            it('should add single item to package', async () => {
                const dto = { items: [{ taskTypeId: 'task-type-uuid-123', quantity: 1 }] };
            ...
            it('should handle item with high quantity', async () => {
                const dto = { items: [{ taskTypeId: 'task-type-uuid-123', quantity: 100 }] };
                const result = await service.addPackageItems('pkg-uuid-123', dto);
            ...
            it('should throw NotFoundException for invalid package id', async () => {
                const dto = { items: [{ taskTypeId: 'task-type-uuid-123', quantity: 1 }] };
                await expect(service.addPackageItems('invalid-id', dto)).rejects.toThrow(NotFoundException);
            });
        });
        ...
        describe('removePackageItem', () => {
            ...
            it('should throw NotFoundException for non-existent item', async () => {
                await expect(service.removePackageItem('invalid-id')).rejects.toThrow(NotFoundException);
            });
        });
    });
});
