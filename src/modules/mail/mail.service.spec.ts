...
describe('MailService', () => {
    ...
    describe('sendBookingConfirmation', () => {
        ...
        it('should catch and log errors', async () => {
            ...
            await expect(service.sendBookingConfirmation(data)).resolves.not.toThrow();
        });
        ...
        it('should handle isEnabled false for all notification types', async () => {
            ...
            await service.sendBookingConfirmation({ clientEmail: 'test@test.com' } as any);
            await service.sendTaskAssignment({ employeeEmail: 'test@test.com' } as any);
            await service.sendPayrollNotification({ employeeEmail: 'test@test.com' } as any);

            expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('[DEV]'), expect.any(Object));
        });
    });
    ...
    describe('sendPayrollNotification', () => {
        ...
        it('should catch and log errors', async () => {
            mockMailerService.sendMail.mockRejectedValueOnce(new Error('Send failed'));
            await expect(service.sendPayrollNotification(data)).resolves.not.toThrow();
        });
    });
});
