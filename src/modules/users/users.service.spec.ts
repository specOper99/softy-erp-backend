...
describe('UsersService - Comprehensive Tests', () => {
    ...
    describe('create', () => {
        ...
        it('should allow creating user with OPS_MANAGER role', async () => {
            const dto = { email: 'ops@example.com', password: 'password123', role: Role.OPS_MANAGER };
            await service.create(dto);
    ...
    describe('update', () => {
        it('should update user email', async () => {
            const result = await service.update('test-uuid-123', { email: 'updated@example.com' });
            expect(mockRepository.save).toHaveBeenCalled();
        });

        it('should update user role', async () => {
            const result = await service.update('test-uuid-123', { role: Role.ADMIN });
            expect(mockRepository.save).toHaveBeenCalled();
        });

        it('should update user active status', async () => {
            const result = await service.update('test-uuid-123', { isActive: false });
            expect(mockRepository.save).toHaveBeenCalled();
        });
