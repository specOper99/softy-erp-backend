...
import { BookingStatus, TaskStatus, TransactionType } from '../src/common/enums';
...
describe('Workflow Integration Tests (E2E)', () => {
    ...
    beforeAll(async () => {
        ...
        const staffLoginRes = await request(app.getHttpServer())
            .post('/auth/login')
            .send({ email: 'john.photographer@chapters.studio', password: 'Staff123!' });
    ...
    describe('Booking Confirmation Workflow', () => {
        ...
        it('should confirm booking and create tasks', async () => {
            ...
            expect(res.body.booking?.status || res.body.status).toBe(BookingStatus.CONFIRMED);
        });
        ...
        it('should have created tasks for the booking', async () => {
            ...
            const bookingTasks = res.body.filter((t: any) => t.bookingId === bookingId);
        ...
        it('should have created income transaction for the booking', async () => {
            ...
            const bookingTransaction = res.body.find(
                (t: any) => t.referenceId === bookingId && t.type === TransactionType.INCOME
            );
